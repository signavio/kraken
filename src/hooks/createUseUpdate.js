...
function createUseUpdate(apiTypes: ApiTypeMap) {
  const actionCreators = createActionCreators(apiTypes)

  function useUpdate<Body, Value>(
    ...
    invariant(
      typeof resolvedType.update === 'function',
      `Invalid method "update" specified for api type "${entityType}"`
    )
...
const usePromiseProp = (actionCreators, entityType, query, requestParams) => {
  ...
  const { dispatchUpdate } = actionCreators
  ...
  return useCallback(
    ...
    [dispatch, dispatchUpdate, entityType, query, requestParams]
  )
}

export default createUseUpdate
